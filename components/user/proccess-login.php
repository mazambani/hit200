...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    //connect to database
    try {
        $errors = array(); // for collecting all the errors
        ...
        if (empty($password)) {
            ...
            if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,12}$/', $password)) {  //                        #8
        ...
        if (empty($errors)) { // If everything's OK.         #1
            ...
            $query = "SELECT id, hashedpassword, firstname, user_level FROM clientdetails ";
            $query .= "WHERE email=?";
        ...
        if (!empty($errors)) {
            ...
            echo "<p class='alert alert-danger'>$errorstring</p>";
