...
if (empty($errors)) { //everything is ok
    try {
        ...
        $query = "SELECT id FROM clientdetails WHERE email = ? ";
        ...
        if (mysqli_num_rows($result) == 0) { //The email address has not been registered
            ...
            //Hash the password current 60 character but it can increase
            $hashedpassword = password_hash($password1trim, PASSWORD_DEFAULT);
            //Make a query
            $query = "INSERT INTO clientdetails (id, firstname, lastname, gender, email, address1, address2, phone, hashedpassword, registrationdate,user_level)";
            $query .= "VALUES (' ',?,?,?,?,?,?,?,?,NOW(),4)";
            $q = mysqli_stmt_init($dbcon);
            mysqli_stmt_prepare($q, $query);
            // use prepared statement to ensure that only text is inserted
            // bind fields to SQL Statement
            mysqli_stmt_bind_param($q, 'ssssssis', $firstnametrim, $lastnametrim, $gender, $emailtrim, $address1trim, $address2trim, $phonetrim, $hashedpassword);
            //Execute the query
            mysqli_stmt_execute($q);
