...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors)) { // If everything's OK.  
            ...
            $query = 'SELECT id FROM admindetails WHERE email=? AND id !=?';
            ...
            if (mysqli_num_rows($result) == 0) { // e-mail does not exist in another record
                ...
                $query = 'UPDATE admindetails 
                    SET firstname=?, lastname=?, gender=?, email=?, address1=?, address2=?, phone=?, experience=?, hashedpassword=?, user_level=1
                    WHERE id=? 
                    LIMIT 1';
                $q = mysqli_stmt_init($dbcon);
                mysqli_stmt_prepare($q, $query);
                // bind values to SQL Statement
                mysqli_stmt_bind_param($q, 'sssssssssi', $firstnametrim, $lastnametrim, $gender, $emailtrim, $address1trim, $address2trim, $phonetrim, $experience, $hashedpassword, $id);
                // execute query
                // execute query
                mysqli_stmt_execute($q);
    ...
    $q = mysqli_stmt_init($dbcon);
    $query = "SELECT id, firstname, lastname, gender, email, address1, address2, phone, experience
            FROM admindetails
            WHERE id=?";
    mysqli_stmt_prepare($q, $query);
    // bind $id to SQL Statement
                            ...
                            <div class="col-md-4 shadow-sm pt-1 m-auto p-md-2">
                                <form action="edit.php" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="number" name="experience" placeholder="Enter experince in years" required value="<?php echo htmlspecialchars($row[8], ENT_QUOTES); ?>">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <input type="submit" value="Register" name="submit_form" class="btn btn-primary my-2 w-100">
                                    </div>
                                </form>
                            </div>
                        </div>
