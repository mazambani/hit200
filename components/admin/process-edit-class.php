...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors)) { // If everything's OK.   
            //check if the name doesnt exist
            //require('../mysqli_connect.php'); //Connect to the db
            $q = mysqli_stmt_init($dbcon);
            $query = 'SELECT id FROM class WHERE name=? AND id !=?';
            ...
            if (mysqli_num_rows($result) == 0) { // name does not exist in another record
                ...
                '<div class="row" style="text-align:center">
                    <p style="color:red">Name already taken</p>
                </div>';
            }
        ...
        <form action="edit-dept.php" method="POST">
            <div class="container">
                <div class="col-md-4 m-auto">
                    ...
                    <div class="form-group">
                        <input class="btn btn-primary w-100 my-2" type="submit" value="submit" name="submit_form">
                    </div>
                </div>
            </div>
