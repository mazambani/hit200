...
if (empty($errors)) { //everything is ok
    try {
        ...
        $query = "SELECT id FROM admindetails WHERE email = ? ";
        ...
        if (mysqli_num_rows($result) == 0) { //The email address has not been registered
            ...
            //Make a query
            $query = "INSERT INTO admindetails (id, firstname, lastname, gender, email, address1, address2,
            phone, experience,user_level, hashedpassword)";
            $query .= "VALUES (' ',?,?,?,?,?,?,?,?,1,?)";
            $q = mysqli_stmt_init($dbcon);
            mysqli_stmt_prepare($q, $query);
            // use prepared statement to ensure that only text is inserted
            // bind fields to SQL Statement
            mysqli_stmt_bind_param($q, 'ssssssiis', $firstnametrim, $lastnametrim, $gender, $emailtrim, $address1trim, $address2trim, $phonetrim, $experience, $hashedpassword);
            //Execute the query
            mysqli_stmt_execute($q);
