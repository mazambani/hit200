...
if ((isset($_POST['id'])) && (is_numeric($_POST['id']))) { // Form submission.      #1
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($sure == 'Yes') { // Delete the record.
            ...
            mysqli_stmt_prepare($q, 'DELETE FROM cardetails WHERE id=? LIMIT 1');
            // bind $id to SQL Statement
            mysqli_stmt_bind_param($q, "i", $id);
            // execute query
            mysqli_stmt_execute($q);
            print_r(mysqli_stmt_affected_rows($q));
            if (mysqli_stmt_affected_rows($q) == 1) { // It ran OK
                // Print a message:
                header("Location: viewcar.php");
            } else { // If the query did not run OK display public 
                print_r(mysqli_stmt_affected_rows($q));
                echo '<p class="text-center">The record could not be delete.';
                echo '<br>Either it does not exist or due to a system.</p>';
        ...
        $q = mysqli_stmt_init($dbcon);
        $query = "SELECT CONCAT(make,' ',model) AS name
                FROM cardetails WHERE id=?";
...
?>
            <div class="row">
                <div class="col-md-4 m-auto">
                    ...
                    <form action="deletecars.php" method="POST" name="deleteform">
                        <div class=" row m-auto">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <input class="btn btn-primary w-100" type="submit" name="sure" value="No">
                                </div>
                            </div>
                    </form>
                </div>
