<?php
try {
    //This script is a query that INSERTs a record in the class records
    ...
    if (empty($errors)) { //This means that everything is okay
        try {
            ...
            if (mysqli_num_rows($result) == 0) { //The  name has not been registered
                ...
                //make a query
                $query = "INSERT INTO class (id, name, description)";
                $query .= "VALUES (' ',?,?)";
                $q = mysqli_stmt_init($dbcon);
                mysqli_stmt_prepare($q, $query);
        ...
        echo '<div class="row">';
        echo '<p style="color:red;margin: auto">' . $errorstring . '</p>';
        echo '</div>';
